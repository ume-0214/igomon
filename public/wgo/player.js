;(function (WGo) {
  ...
  var detect_scrolling = function (node, bp) {
    if (node == bp.element || node == bp.element) return false
      ...
      node._wgo_scrollable ||
      (node.scrollHeight > node.offsetHeight && window.getComputedStyle(node).overflow == 'auto')
    )
      return true
  ...
  var board_click_default = function (x, y) {
    ...
    for (var i in this.kifuReader.node.children) {
      if (
        this.kifuReader.node.children[i].move &&
        this.kifuReader.node.children[i].move.x == x &&
        this.kifuReader.node.children[i].move.y == y
      ) {
        this.next(i)
        return
      }
  ...
  Player.prototype = {
    ...
    loadKifu: function (kifu, path) {
      ...
      this.kifuReader = new WGo.KifuReader(
        this.kifu,
